...
### 项目管理功能完善
- **新增项目管理页面**：完整的项目列表、创建、编辑、详情查看功能
- **项目种子数据**：自动生成 6 个示例项目及参与人员数据
- **代码架构优化**：将项目相关 payload 校验逻辑分离到独立的 `project_payloads.py` 模块
- **设计思路调整**：移除项目基础信息中的 `our_project_manager` 字段，改为通过 `person_project_history` 中的 `project_position` 来记录项目经理
- **Bug 修复**：修复了所有模态框中的标签重叠问题，确保标签正确显示

### 技术改进
- 服务层添加了 `get_current_project_manager()` 方法，从参与人员记录中查询当前项目经理
- 前端优化了项目经理显示逻辑，在参与人员列表中高亮显示项目经理
- 统一修复了所有模态框的 Materialize CSS 标签动画问题
