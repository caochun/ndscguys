...
一个面向“人员状态追踪”的原型系统，使用 Flask + SQLite 实现 **State Stream（状态流）** 模式：以 `person_id` 为聚合根，把基础信息、岗位信息、薪资、社保、公积金、考核、发薪记录等拆解为独立的 Append-only 流，每次变更只追加新版本，不改写历史，再通过 Web UI + API 展示当前状态及历史沿革。
